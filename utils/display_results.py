...
def show_performance(pos, neg, method_name='Ours', recall_level=recall_level_default, logger=None):
    '''
...
def print_measures_with_std(aurocs, auprs, fprs, method_name='Ours', recall_level=recall_level_default, logger=None):
    logger.info('\t\t\t\t' + method_name)
    logger.info(
        'FPR{:d}:\t\t\t{:.2f}\t+/- {:.2f}'.format(int(100 * recall_level), 100 * np.mean(fprs), 100 * np.std(fprs)))
