...
class MMD(nn.Module):
    ...
    def compute_kernel_dist(self, dist, gamma, kernel_num, kernel_mul):
        ...
        gamma_tensor = torch.tensor(gamma_list).cuda()
    ...
    def kernel_layer_aggregation(self, dist_layers, gamma_layers):
        ...
        for i in range(num_layers):
            ...
            if len(kernel_dist.keys()) == 0:
                kernel_dist = {
                    key: self.compute_kernel_dist(
                        dist[key], gamma, cur_kernel_num, cur_kernel_mul)
                    for key in ['ss', 'tt', 'st']}
                continue

            if not self.joint:
                kernel_dist = {
                    key: kernel_dist[key] + self.compute_kernel_dist(
                        dist[key], gamma, cur_kernel_num, cur_kernel_mul)
                    for key in ['ss', 'tt', 'st']}
